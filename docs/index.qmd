...
Aus beiden figrues (@fig-initiatives, @fig-yesinitiatives) geht hervor, dass the "Net Zero initiative" is the most known (66% of participants heard of it), followed by the SDG Public Lecture Series and the Sustainable Gastronomy Project (33% of participants heard of it respectively).
...
daher wichtig zu sehen, wo die Teilnehmer die Informationen erhalten, um festzustellen welches Medium für die Informationdiffusion besonders
effektiv ist.
...
konnte, die zumindest von der Net Zero initiative gehört haben.
Daher wurden in einem letzten Schritt alle Teilnehmer gefragt, welcher Channel am geeignetsten wäre, um die Visibility des ETH Departments of
...
channel_labels <- c(
  ...
  notice_ETH_mailing_list = "ETH intranet",
...
visibility_counts <- eth_dep_sus_visibility_visual %>%
  select(starts_with("notice_")) %>%
  summarise_all(~sum(. == "Yes", na.rm = TRUE) / n()) %>%
  gather(key = "Channel", value = "Percentage") %>%
  mutate(Channel = recode(Channel, !!!channel_labels),
         Channel = str_wrap(Channel, width = 30))

# Results display as visualization --------------------------------------
ggplot(visibility_counts, aes(
  x = reorder(Channel, -Percentage), 
  y = Percentage)) +
  geom_bar(stat = "identity", fill = "#2a9d8f", width = 0.8) +  # Adjust bar width and color
  geom_text(aes(
    label = scales::percent(Percentage, accuracy = 1)), 
